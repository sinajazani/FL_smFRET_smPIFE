...
figure('Name','Transition rates + FRET rates, Donor and acceptor lifetimes','Units','inch','Position',[0 0 7 4])
set(gcf,'color','w')
clf
movegui(gcf,'center')
...
bb7=subplot(3,4,8);
maxx=0;
TT=linspace(0,max(max(Data.KD_val_learned{mmk}(burn_in:end,:))),100);
for nm=1:Data.Num_states_D 
...
bb7=subplot(3,4,12);
maxx=0;
TT=linspace(0,max(max(Data.KA_val_learned{mmk}(burn_in:end,:))),100);
for nm=1:Data.Num_states_A 
